...
Đây là một codebase **được xây dựng rất kỷ luật về mặt trung thực dữ liệu**. Phần lớn "dữ liệu giả" ở đây **không phải lỗi** mà là placeholder có chủ đích, được xử lý phòng thủ: khi chưa cấu hình, UI tự ẩn đi ("Đang cập nhật"), nút CTA tự vô hiệu hoá, JSON-LD tự lược bỏ, và mọi nội dung mẫu đều gắn cờ `isPlaceholder`. Vì vậy đa số phát hiện thuộc nhóm _cần cung cấp thông tin thật_ hơn là _phải sửa gấp một lỗi nguy hiểm_.
...
_Không ghi giá trị đầy đủ của secret. `AUTH_SECRET` đã được đặt giá trị ngẫu nhiên 57 ký tự — đạt yêu cầu, không liệt kê ở đây như blocker._
...
- `tuition: null` ở mọi khóa → hiển thị `siteConfig.messaging.feeNotConfigured` = _"Vui lòng liên hệ để nhận thông tin học phí và lịch khai giảng…"_.
